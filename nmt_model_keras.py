from keras.layers import Embedding, LSTM, Dropout, Dense, Layer, Softmax, Multiply
...
class AttentionLayer(Layer):
    ...
    def call(self, inputs, mask=None):
        ...
        # print(encoder_outputs.shape) # (None, None, 200)
        # print(decoder_outputs.shape) # (None, None, 200)
        ...
        Start
        """
        # Permute the dimensions of decoder_outputs

        # hidden states of encoder - [batch_size, max_source_sent_len, hidden_size]
        # hidden states of decode -  [batch_size, max_target_sent_len, hidden_size].
        # Attention scores - [batch_size, max_source_sent_len, max_target_sent_len, 1]

        permutation = (0, 2, 1)
        decoder_outputs = tf.keras.backend.permute_dimensions(decoder_outputs, permutation)
        # decoder_outputs.transpose(0,2,1)
        luong_score = tf.keras.backend.batch_dot(encoder_outputs, decoder_outputs)
        attention_scores = tf.nn.softmax(luong_score, axis=1)
            #Softmax(axis=1)(luong_score)  # normalizes values along axis 2
        #print("luong_score: ", luong_score.shape)
        #print("attention_scores: ", attention_scores.shape)

        encoder_outputs = tf.expand_dims(encoder_outputs, 2)
        attention_scores = tf.expand_dims(attention_scores, 3)

        # print("luong_score, ", luong_score.shape)
        # print("attention_scores, ", attention_scores.shape)

        encoder_vector= tf.multiply(encoder_outputs, attention_scores)
        encoder_vector = tf.reduce_sum(encoder_vector, 1)
        """
        End Task 3
        """
        # [batch,max_dec,2*emb]
        new_decoder_outputs = K.concatenate([inputs[1], encoder_vector])
        print(new_decoder_outputs.shape)
...
class NmtModel(object):
    def __init__(self, source_dict, target_dict, use_attention):
        ...
        # TODO: We added this, maybe it's wrong
        self.max_source_step = 30
    ...
    def build(self):
        ...
        #len(source_words.shape[1]) = [batch_size, max_sequence_length ]
        """
        Task 1 encoder
        ...
        # -----------Write your code here: no input length on source language?--------
        embeddings_source = Embedding(self.vocab_source_size, self.embedding_size, embeddings_initializer='random_uniform',
                                      mask_zero=True)
        #input_length=source_words.shape[1]
        # self.max_source_step,
        embeddings_target = Embedding(self.vocab_target_size, self.embedding_size,embeddings_initializer='random_uniform',
                                      mask_zero=True)
        ...
        source_word_embd_drop = Dropout(self.embedding_dropout_rate)(source_word_embeddings)

        target_words_embeddings = embeddings_target(target_words)
        target_word_embd_drop = Dropout(self.embedding_dropout_rate)(target_words_embeddings)

        # (c.) An encoder LSTM() with return sequences set to True
        print('\nTask 1(c): Creating an encoder')
        encoder_lstm = LSTM(self.hidden_size, recurrent_dropout=self.hidden_dropout_rate, return_sequences=True,
                            return_state=True)
        encoder_outputs, encoder_state_h, encoder_state_c = encoder_lstm(source_word_embd_drop)
        ...
        decoder_lstm = LSTM(self.hidden_size, recurrent_dropout=self.hidden_dropout_rate, return_sequences=True,
                            return_state=True)
        print("target_word_embd_drop shape = ", target_words_embeddings.shape)
        decoder_outputs_train, _, _ = decoder_lstm(target_word_embd_drop,
                                                   initial_state=encoder_states)  # target_words_embeddings
        print("B4")
        print(decoder_outputs_train.shape)
        ...
        print('After', decoder_outputs_train.shape)
    ...
    def create_translation(self, source_sentence):
        # get the source words and target_word_labels for the eval dataset
        source_words, target_words_labels = source_sentence
        ...
        return self.get_target_sentences(np.concatenate(predictions, axis=1), vocab)
    ...
    def eval(self, dataset):
        ...
        # using the same encoding network used during training time, encode the training
        encoder_outputs, state_h, state_c = self.encoder_model.predict(source_words, batch_size=self.batch_size)
        # encoder_outputs1, state_h1, state_c1 = self.encoder_model.predict(sample_sent, batch_size=self.batch_size)
...
def main(source_path, target_path, use_attention):
    ...
    model.build()
    model.train(train_data, dev_data, test_data, 6)

    print('Sentence 1: ', model.get_target_sentences(train_data[0], target_dict))
    print('Sentence 2: ', model.get_target_sentences(train_data[1], target_dict))